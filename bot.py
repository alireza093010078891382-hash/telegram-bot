...
TOKEN = "8989518912:AAFP95yajzcrVOumBw_iay3kZBpardwQ2zM"  
ADMIN_ID = 7815449425  
...
def get_auth_menu(lang):
    if lang == "fa":
        return InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text="📝 ثبت‌نام", callback_data="auth_register")],
            [InlineKeyboardButton(text="🔑 ورود به حساب", callback_data="auth_login")],
            [InlineKeyboardButton(text="🔄 بازیابی حساب (مفتاح)", callback_data="auth_recovery")]
        ])
    else:
        return InlineKeyboardMarkup(inline_keyboard=[
...
async def lang_callback(callback: CallbackQuery):
    ...
    prompt = (
        "🔐 **لطفاً یکی از گزینه‌های زیر را انتخاب کنید:**" if lang == "fa"
...
async def auth_register(callback: CallbackQuery):
    ...
    prompt_text = (
        "🇮🇷 لطفاً یک **نام کاربری (Username)** انگلیسی منحصربه‌فرد وارد کنید:" if lang == "fa" 
        else "🇺🇸 Please enter your unique **Username**:"
    )
...
async def auth_login(callback: CallbackQuery):
    ...
    prompt_text = (
        "🔑 **ورود به حساب کاربری**\n\nلطفاً **نام کاربری** ثبت‌نام شده خود را وارد کنید:" if lang == "fa"
...
async def auth_recovery(callback: CallbackQuery):
    ...
    prompt_text = (
        "🔄 **بازیابی حساب کاربری**\n\nلطفاً **۱۵ کلمه عبارت بازیابی امن (Seed Phrase)** خود را ارسال کنید تا حساب شما بازیابی شود:" if lang == "fa"
        else "🔄 **Account Recovery**\n\nPlease send your **15-word recovery seed phrase** to restore your account:"
    )
    await callback.message.edit_text(prompt_text, reply_markup=get_back_menu(lang), parse_mode="Markdown")
    await callback.answer()
...
async def referral_menu(callback: CallbackQuery):
    ...
    kb = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="💎 پاداش دعوت (Rewards)" if lang == "fa" else "💎 Rewards", callback_data="ref_reward_info")],
        [InlineKeyboardButton(text="🔗 لینک اختصاصی دعوت" if lang == "fa" else "🔗 Referral Link", callback_data="ref_get_link")],
        [InlineKeyboardButton(text="👥 مشاهده دعوت‌شدگان من" if lang == "fa" else "👥 My Referrals", callback_data="ref_list")],
...
async def handle_steps(message: Message):
    ...
    if state == "get_recovery_seed":
        ...
        if not user_data:
            err_msg = (
                "❌ **عبارت بازیابی نامعتبر است!** حسابی با این ۱۵ کلمه پیدا نشد. لطفاً دوباره دقت کرده و ارسال کنید:" if lang == "fa"
                else "❌ **Invalid Seed Phrase!** No account found with these 15 words. Please try again:"
            )
        ...
        success_login = (
            "✅ **حساب کاربری شما با موفقیت بازیابی شد و وارد شدید!**" if db_lang == "fa"
            else "✅ **Account successfully recovered!**"
        )
    ...
    if state == "get_login_username":
        ...
        pwd_prompt = (
            "🔑 حالا **رمز عبور** حساب خود را وارد کنید:" if lang == "fa"
            else "🔑 Now enter your **Password**:"
        )
    ...
    if state == "get_login_password":
        ...
        if not user_data:
            err_msg = (
                "❌ **نام کاربری یا رمز عبور اشتباه است!** لطفاً دوباره تلاش کنید یا از گزینه «فراموشی حساب» استفاده کنید:" if lang == "fa"
                else "❌ **Invalid Username or Password!** Please enter correct credentials or use Account Recovery:"
            )
            await message.answer(err_msg, parse_mode="Markdown")
            return
        ...
        success_login = (
            "✅ **با موفقیت به حساب کاربری خود وارد شدید!**" if db_lang == "fa"
    ...
    if state == "get_username":
        if len(text) < 3:
            err = "❌ نام کاربری باید حداقل ۳ کاراکتر باشد. لطفاً دوباره وارد کنید:" if lang == "fa" else "❌ Username must be at least 3 characters."
            await message.answer(err)
            return
        
        if is_username_taken(text) and not user_states[uid].get("is_editing"):
            err = "⚠️ این نام کاربری قبلاً ثبت شده است! لطفاً نام کاربری دیگری انتخاب کنید:" if lang == "fa" else "⚠️ Username is already taken! Try another:"
            await message.answer(err)
            return
        ...
        pwd_prompt = (
            "🔑 عالی بود! حالا لطفاً یک **رمز عبور (Password)** قوی وارد کنید.\n\n"
            "⚠️ **الزامات رمز عبور:**\n"
            " • حداقل ۱ حرف بزرگ انگلیسی (A-Z)\n"
            " • حداقل ۱ حرف کوچک انگلیسی (a-z)\n"
            " • حداقل ۱ عدد (0-9)\n"
            " • حداقل ۱ علامت خاص (مثل @, #, $, !)" if lang == "fa"
            else "🔑 Great! Now enter a strong **Password**:\n\n"
            "⚠️ **Requirements:**\n"
        ...
        if not (re.search(r"[A-Z]", text) and re.search(r"[a-z]", text) and re.search(r"[0-9]", text) and re.search(r"[!@#$%^&*(),.?\":{}|<>]", text)):
            err = (
                "❌ رمز عبور فاقد الزامات امنیتی است! باید شامل حرف بزرگ، حرف کوچک، عدد و علامت خاص باشد:" if lang == "fa"
                else "❌ Password security requirements not met! Must contain uppercase, lowercase, number, and special character:"
            )
            await message.answer(err)
            return
        ...
        if user_states[uid].get("is_editing"):
            ...
            success_edit = "✅ نام کاربری و رمز عبور با موفقیت بروزرسانی شد!" if curr_lang == "fa" else "✅ Credentials updated successfully!"
        ...
        if not success:
            err = "❌ خطا در ثبت‌نام. لطفاً مجدداً /start را بفرستید." if lang == "fa" else "❌ Registration error. Please send /start again."
            await message.answer(err)
            del user_states[uid]
            return
        ...
        success_msg = (
            "✅ **حساب کاربری شما با موفقیت و بالاترین سطح امنیت ساخته شد!**\n\n"
            "🗝 **عبارت بازیابی ۱۵ کلمه‌ای شما (Seed Phrase):**\n"
            f"`{seed_phrase}`\n\n"
            "⚠️ **هشدار مهم:** این ۱۵ کلمه را در جای امن یادداشت کنید. اگر روزی رمز عبورتان را فراموش کردید، با این کلمات می‌توانید حساب خود را بازیابی کنید!" if lang == "fa"
            else "✅ **Account created successfully with maximum security!**\n\n"
            "🗝 **Your 15-Word Recovery Seed Phrase (Save it securely):**\n"
            f"`{seed_phrase}`\n\n"
            "⚠️ **Important:** Save these 15 words securely. If you forget your password or lose access, this is your recovery key!"
        )
...
async def edit_settings(callback: CallbackQuery):
    ...
    prompt = "⚙️ **ویرایش حساب کاربری**\n\nلطفاً نام کاربری جدید خود را وارد کنید:" if lang == "fa" else "⚙️ **Edit Credentials**\n\nEnter your new **Username**:"
...
async def main():
    init_db()
    print("Bot is running with corrected Persian translation support and new token!")
